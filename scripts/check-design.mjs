...
const checks = [
  ...
  ["four user palettes", ["botanical", "ocean", "plum", "cobalt"].every((palette) => app.includes(`id: "${palette}"`))],
  ["palette CSS themes", ["ocean", "plum", "cobalt"].every((palette) => css.includes(`data-palette="${palette}"`))],
  ...
  ["theme choice survives reloads", app.includes('localStorage.setItem("cluster_theme", isDark ? "dark" : "light")') && html.includes("2026-07-user-kpi-analytics")],
