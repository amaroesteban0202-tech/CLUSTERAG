...
const isValidVapidPublicKey = (value) => /^[A-Za-z0-9_-]{80,120}$/.test(
    String(value || '').trim()
);
...
export const registerFirebaseWebPush = async ({ onMessage } = {}) => {
    if (
    ...
    if (!isValidVapidPublicKey(vapidKey)) {
        throw new Error('La clave pública Web Push no es válida.');
    }
